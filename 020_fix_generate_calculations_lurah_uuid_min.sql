-- Migration 020: Fix Generate Calculations Lurah UUID Min
-- Deskripsi: Menghapus error MIN(uuid) yang tidak didukung Postgres.
...
BEGIN
    ...
    -- Verifikasi Keberadaan Tepat 1 Penugasan Aktif Lurah
    SELECT count(*)
    INTO v_lurah_count
    FROM public.employees e
    JOIN public.employee_position_assignments epa ON epa.employee_id = e.id
    ...
    IF v_lurah_count = 1 THEN
        SELECT e.id, epa.position_id
        INTO v_lurah_emp_id, v_lurah_pos_id
        FROM public.employees e
        JOIN public.employee_position_assignments epa ON epa.employee_id = e.id
        JOIN public.positions pos ON pos.id = epa.position_id
        WHERE pos.name = 'Lurah' 
