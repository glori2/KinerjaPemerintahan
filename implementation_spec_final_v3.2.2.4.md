# IMPLEMENTATION SPECIFICATION FINAL V3.2.2.4
— FINAL MICRO PATCH: PROFILE PROVISIONING CLOSURE

Dokumen ini merupakan spesifikasi teknis definitif (*Final Micro Patch: Profile Provisioning Closure*) sebelum memasuki fase penulisan kode migrasi SQL Supabase/PostgreSQL. Pemutakhiran ini secara khusus menghapus seluruh ambiguitas terkait pembuatan profil pengguna dengan menutup mutlak hak `INSERT` langsung dari peran klien (`authenticated`, Carik, Lurah, maupun Pamong), serta mengukuhkan bahwa pembentukan profil dan penugasan peran (*role*) berada 100% di bawah kendali *Trusted Provisioning Context*.

---

## 1. Executive Summary
Spesifikasi V3.2.2.4 melengkapi V3.2.2.3 dengan penegasan arsitektural mutlak pada pembuatan identitas:
1. **Penutupan Total Direct Client INSERT pada `profiles`**: Klien biasa (`authenticated`, Carik, Lurah, Pamong) tidak memiliki hak `INSERT` langsung pada `public.profiles`. Seluruh pembuatan profil hanya dapat dilakukan melalui *Trusted Provisioning*.
2. **Pemberantasan Konsep "Coercion"**: Menghapus seluruh mekanisme *Before Insert Trigger Coercion*; tidak ada proses di mana klien melakukan insert lalu role di-coerce oleh pemicu basis data. Klien murni ditolak (*DENY*) melakukan insert.
3. **Pemberian Role Sepenuhnya Terpercaya**: Nilai `profiles.role` ('admin' | 'user') hanya dapat ditetapkan oleh proses *Trusted Provisioning* server-side yang terotorisasi.
4. **Pembatasan Hak Akses `provision_employee_identity`**: Fungsi provisioning tidak diekspos ke `PUBLIC` atau `authenticated`, melainkan dilindungi dengan `REVOKE EXECUTE` dan hanya dapat dipanggil oleh *service context* terpercaya.
5. **Konsistensi Lapis Pertahanan**: Integrasi `GRANT/REVOKE`, RLS, dan Trigger *defense-in-depth* yang selaras tanpa bergantung pada *session variable* palsu (`set_config`).
...
1. **`profiles`**
   ...
   - `role` (varchar: 'admin' | 'user') **[DATABASE ENFORCEMENT - PATCH 01 & 02]**:
     - Direct client `INSERT` = **DENY**. Tidak ada hak `INSERT ON public.profiles` untuk `authenticated`.
     - Direct client `UPDATE (role)` = **DENY**. `REVOKE UPDATE (role) ON public.profiles FROM authenticated;`
     - Role hanya dapat ditetapkan dan diubah melalui fungsi *Trusted Provisioning*.
   - `full_name` (varchar)
...
3. **`employees`**
   - `id` (uuid, PK)
   - `profile_id` (uuid, FK profiles.id, RESTRICT, UNIQUE) **[DATABASE ENFORCEMENT - PATCH 03]**:
     ...
     - Direct client `INSERT` = **DENY**. Pengikatan `profile_id` hanya dilakukan saat pembentukan identitas oleh *Trusted Provisioning*.
...
## 4. Trust Boundary & Provisioning Architecture **[FINAL MICRO PATCH]**

### A. Penutupan Mutlak Direct Client Provisioning
Direct client `INSERT` terhadap tabel `public.profiles` dan `public.employees` berstatus **DENY MUTLAK**.
- `authenticated` tidak diberi wewenang `INSERT` ke `public.profiles`.
- Carik, Lurah, maupun Pamong tidak dapat menyuntikkan profil baru secara langsung melalui kueri SQL klien atau endpoint PostgREST.
- Tidak ada mekanisme *coercion* (tidak ada konsep insert dengan role apa saja lalu diubah paksa oleh trigger). Klien yang mencoba melakukan direct `INSERT` akan langsung ditolak oleh PostgreSQL permission level.

### B. Prosedur Trusted Provisioning
Seluruh pembentukan profil dan pengikatan ke pegawai dilakukan secara atomik melalui satu prosedur terisolasi (`public.provision_employee_identity`):
1. Membuat entitas `public.profiles` dengan identitas `auth.users.id`.
2. Menetapkan `public.profiles.role` secara eksplisit dan otoritatif ('admin' atau 'user').
3. Membuat entitas `public.employees` dengan mengikat `profile_id`.
4. Memverifikasi secara unik bahwa `profile_id` belum pernah dipakai (1:1 mutlak).
5. Membuat entitas `public.employee_position_assignments` awal sesuai penugasan resmi.

### C. Pembuktian Konteks Terpercaya (No Fake Flags)
- Prosedur provisioning dilindungi secara kriptografis dan hak akses PostgreSQL:
  `REVOKE EXECUTE ON FUNCTION public.provision_employee_identity FROM PUBLIC, authenticated;`
  `GRANT EXECUTE ON FUNCTION public.provision_employee_identity TO service_role;`
- Sistem **DILARANG** mengandalkan parameter sesi palsu seperti `set_config('app.is_trusted', ...)`. Eksekusi hanya dapat dicapai melalui Auth Webhook Supabase, trigger internal pendaftaran akun `auth.users`, atau RPC berprivilese `service_role`.

---

## 5. Security Enforcement Matrix **[FINAL MICRO PATCH]**

| Operasi | `authenticated` (Pamong) | Carik (Admin Workflow) | Lurah (Evaluator) | Trusted Provisioning (`service_role`) |
|---|---|---|---|---|
| **INSERT `profiles`** | **DENY** | **DENY** | **DENY** | **ALLOW** |
| **Set `profiles.role`** | **DENY** | **DENY** | **DENY** | **ALLOW** |
| **UPDATE `profiles.role`** | **DENY** | **DENY** | **DENY** | **ALLOW** |
| **DELETE `profiles`** | **DENY** | **DENY** | **DENY** | Controlled Only |
| **INSERT `employees`** | **DENY** | **DENY direct** | **DENY** | **ALLOW** |
| **Set `employees.profile_id`** | **DENY** | **DENY** | **DENY** | **ALLOW on creation** |
| **UPDATE `employees.profile_id`** | **DENY** | **DENY** | **DENY** | Controlled Only |
| **UPDATE Ordinary Profile Fields** (`full_name`) | Sesuai RLS (Self) | ALLOW | ALLOW | ALLOW |
| **UPDATE Ordinary Employee Fields** (`nip_nipt`) | Sesuai RLS (Self) | ALLOW | ALLOW | ALLOW |

*Catatan: Carik adalah Administrator alur kerja aplikasi, bukan database superuser penyedia identitas.*
...
## 12. Assessment Authorization & Cross-Reference Security
...
## 21. RLS & Privileges Operation Matrix Final **[FINAL MICRO PATCH]**

Menerapkan tiga lapis konsistensi mutlak: **RLS + GRANT/REVOKE + Trigger Exception**:

| Tabel / Domain | Pamong (`authenticated`) | Carik (Admin) | Lurah (Evaluator) | Trusted Provisioning (`service_role`) |
|---|---|---|---|---|
| `profiles` | SELECT (Self)<br>INSERT = **DENY**<br>UPDATE = Self (Ordinary) | SELECT (All)<br>INSERT = **DENY**<br>UPDATE (Admin, **DENY `role`**)<br>DELETE = **DENY** | SELECT (All)<br>INSERT = **DENY**<br>DELETE = **DENY** | SELECT, INSERT, UPDATE, DELETE |
| `employees` | SELECT (Self)<br>INSERT = **DENY** | SELECT (All)<br>INSERT = **DENY direct**<br>UPDATE (Admin, **DENY `profile_id`**)<br>DELETE = **DENY** | SELECT (All)<br>INSERT = **DENY**<br>DELETE = **DENY** | SELECT, INSERT, UPDATE, DELETE |
...
## 22. RPC Authorization Matrix & Security Definer Hardening **[FINAL MICRO PATCH]**
...
1. **`public.submit_journal(p_journal_id uuid)`**
   - *EXECUTE*: `GRANT` ke `authenticated`.
...
6. **`public.generate_calculations(p_period_id uuid)`**
   - *EXECUTE*: `GRANT` ke `authenticated` (Carik) dan `service_role`.
...
7. **Fungsi Internal Provisioning Identity (`public.provision_employee_identity(...)`)**
   - *EXECUTE*: **REVOKE DARI `PUBLIC` DAN `authenticated`**. Hanya diberikan kepada `service_role` (Auth Trigger / Server Provisioning).
...
## 30. Security Enforcement Requirements **[FINAL MICRO PATCH]**
...
1. **Direct Client INSERT `profiles` & Role Protection**:
   - `NO INSERT ON public.profiles` untuk peran `authenticated`.
   - `REVOKE UPDATE (role) ON public.profiles FROM authenticated;`
   - Trigger `BEFORE UPDATE` pada `public.profiles` melempar eksepsi bila ada percobaan mutasi `role`.
   - Seluruh pembentukan profil hanya dapat dilakukan melalui fungsi `public.provision_employee_identity`.
2. **Imutabilitas `employees.profile_id`**:
   - `NO INSERT/UPDATE` langsung dari klien pada kolom `profile_id`.
   - Trigger `BEFORE UPDATE` pada `public.employees` melempar eksepsi jika `NEW.profile_id IS DISTINCT FROM OLD.profile_id`.
   - Constraint `UNIQUE(profile_id)` menjamin 1:1 mutlak.
3. **Konfigurasi `search_path` & Schema-Qualification**:
   ...
   - Seluruh kueri internal menggunakan objek schema-qualified (`public.tukin_periods`, `public.profiles`, dll.).
4. **Pembatasan Hak Panggil (EXECUTE Privileges)**:
   - `REVOKE EXECUTE ON ALL FUNCTIONS IN SCHEMA public FROM PUBLIC;`
   - Hanya fungsi workflow yang di-GRANT ke `authenticated`, sedangkan fungsi provisioning (`provision_employee_identity`) hanya di-GRANT ke `service_role`.
...
## 31. Final Self-Audit (12 Enforcement Points) **[FINAL MICRO PATCH]**

| Temuan | Enforcement | Status |
|---|---|---|
| Client INSERT profiles | **DENY** (No INSERT grant to authenticated) | **CLOSED** |
| Carik INSERT profiles direct | **DENY** (Carik operates via workflow, not direct DB provisioning) | **CLOSED** |
| Client set role | **DENY** (Role determined solely by trusted provisioning) | **CLOSED** |
| Client update role | **DENY** (REVOKE UPDATE (role) + Trigger Exception) | **CLOSED** |
| Client set profile_id | **DENY** (Profile_id set solely by trusted provisioning) | **CLOSED** |
| Client update profile_id | **DENY** (REVOKE UPDATE (profile_id) + Trigger Exception) | **CLOSED** |
| Trusted provisioning | Isolated to `service_role` execution context | **CLOSED** |
| Fake trust/session flag | Prohibited (No reliance on session variables/set_config) | **CLOSED** |
| Provisioning EXECUTE | Restricted (`REVOKE EXECUTE ... FROM authenticated, PUBLIC`) | **CLOSED** |
| SECURITY DEFINER search_path | Hardened (`SET search_path = pg_catalog, public`) | **CLOSED** |
| Schema-qualified objects | Required (`public.<table>` explicitly referenced) | **CLOSED** |
| RLS + GRANT/REVOKE + Trigger | Consistent (Multi-layer defense-in-depth) | **CLOSED** |

---

## 32. Final SQL-Readiness Gate

- [x] Client tidak dapat INSERT profiles
- [x] Client tidak dapat menentukan profiles.role
- [x] Client tidak dapat UPDATE profiles.role
- [x] Carik tidak dapat membuat profile secara direct
- [x] Client tidak dapat mengubah employees.profile_id
- [x] Trusted provisioning terisolasi
- [x] Trusted provisioning tidak menggunakan fake session flag
- [x] Provisioning EXECUTE restricted
- [x] SECURITY DEFINER hardened
- [x] Search path aman (`pg_catalog, public`)
- [x] Sensitive objects schema-qualified (`public.<table>`)
- [x] RLS + GRANT/REVOKE + Trigger konsisten
- [x] Tidak ada business rule berubah
- [x] Bamuskal tetap **TOTAL OUT OF SCOPE**
- [x] Deferred policy tetap deferred
...
---
*Spesifikasi V3.2.2.4 telah disahkan secara mutlak. Seluruh ambiguitas pembuatan identitas telah ditutup dan sistem siap memasuki tahap SQL Migration.*
