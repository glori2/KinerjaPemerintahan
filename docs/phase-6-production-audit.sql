...
SELECT p.name as position_name, mv.version_number, mv.status, mv.effective_from, mv.effective_to FROM matrix_versions mv JOIN positions p ON mv.position_id = p.id WHERE mv.status = 'Published' ORDER BY p.name;  
SELECT mi.name, mt.monthly_target, mt.annual_target FROM performance_items mi JOIN performance_targets mt ON mi.id = mt.item_id LIMIT 10;  
SELECT p.name as position_name, tp.pagu_tukin, tp.min_ckb_target, tp.effective_from FROM tukin_parameters tp JOIN positions p ON tp.position_id = p.id WHERE tp.effective_from <= '2027-01-01' ORDER BY tp.effective_from DESC;  
